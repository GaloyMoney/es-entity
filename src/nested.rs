...
/// Manages nested entities within a parent entity and maintaining child-parent relationships
///
...
impl<T: EsEntity> Nested<T> {
    /// Adds a new entity to the collection of entities awaiting persistence.
    ...
    /// Retrieves an immutable reference to a persisted entity by its ID.
    pub fn get_persisted(&self, id: &<<T as EsEntity>::Event as EsEvent>::EntityId) -> Option<&T> {
        self.entities.get(id)
    }

    /// Retrieves a mutable reference to a persisted entity by its ID.
    ...
    /// Finds the first persisted entity matching the provided predicate.
    ...
    /// Finds the first persisted entity matching the provided predicate, returning a mutable reference.
    ...
    /// Returns the number of persisted entities in the collection.
    pub fn len_persisted(&self) -> usize {
        self.entities.len()
    }

    /// Returns a mutable iterator over all persisted entities.
    ...
    /// Returns a mutable reference to the collection of new entities awaiting persistence.
    pub fn new_entities_mut(&mut self) -> &mut Vec<<T as EsEntity>::New> {
        &mut self.new_entities
    }

    /// Loads persisted entities into the collection, typically during entity hydration, grouping by entity's ID
...
/// Trait that entities implement for every field marked `#[es_entity(nested)]`, essentially marking them
/// as `Parent` entities for the child entity `T`
///
