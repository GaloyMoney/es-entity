...
/// generated functions like `list_by`, `list_for`, `list_for_filter` and `list_for_filters`. Has two variants: `Ascending` and `Descending`
///
/// # Examples
///
/// ```ignore
...
///
/// Sort enum is used to specify the sorting order and the field to sort the entities by when listing them using `list_for_filter` or `list_for_filters`
...
/// Returned by the [`EsRepo`][crate::EsRepo] functions like `list_by`, `list_for`, `list_for_filter` and `list_for_filters`.
