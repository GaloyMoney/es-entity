        ...
        tokens.append_all(quote! {
                ...
                OP: es_entity::AtomicOperation
            {
                let __result: Result<Vec<#entity>, #error> = async {
                    ...
                    let mut all_event_refs: Vec<&mut es_entity::EntityEvents<<#entity as es_entity::EsEntity>::Event>> = all_events.iter_mut().collect();
                    let mut n_persisted = self.persist_events_batch(op, &mut all_event_refs).await?;
