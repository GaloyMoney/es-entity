...
impl ToTokens for PersistEventsBatchFn<'_> {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        ...
        let query = format!(
            "INSERT INTO {} (id, recorded_at, sequence, event_type, event, context) \
             SELECT unnested.id, COALESCE($1, NOW()), unnested.sequence, unnested.event_type, unnested.event, unnested.context \
             FROM UNNEST($2, $3::INT[], $4::TEXT[], $5::JSONB[], $6::JSONB[]) \
             AS unnested(id, sequence, event_type, event, context) RETURNING recorded_at",
        ...
        tokens.append_all(quote! {
                ...
                OP: es_entity::AtomicOperation
            {
                ...
                for events in all_events.iter_mut() {
                    ...
                    let (serialized, contexts) = events.serialize_new_events();
                ...
                let all_contexts = if all_contexts.is_empty() {
                    None
                } else {
                     Some(all_contexts)
                };
