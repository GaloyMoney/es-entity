...
impl ToTokens for PersistEventsBatchFn<'_> {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        ...
        let query = format!(
            ...
            if context { ", $6::JSONB" } else { "" },
        );

        let context_bind = if context {
            quote! { .bind(Self::current_context()) }
        } else {
            quote! {}
        };
...
mod tests {
    ...
    #[test]
    fn persist_events_batch_fn_with_context() {
        let id = syn::parse_str("EntityId").unwrap();
        let event = syn::Ident::new("EntityEvent", proc_macro2::Span::call_site());
        let error = syn::parse_str("es_entity::EsRepoError").unwrap();
        let persist_fn = PersistEventsBatchFn {
        ...
        let expected = quote! {
                ...
                OP: es_entity::AtomicOperation
            {
                ...
                let rows = Self::extract_concurrent_modification(
                    sqlx::query("INSERT INTO entity_events (id, recorded_at, sequence, event_type, event, context) SELECT unnested.id, COALESCE($1, NOW()), unnested.sequence, unnested.event_type, unnested.event, $6::JSONB FROM UNNEST($2, $3::INT[], $4::TEXT[], $5::JSONB[]) AS unnested(id, sequence, event_type, event) RETURNING recorded_at")
                        .bind(now)
                        .bind(&all_ids)
                        .bind(&all_sequences)
                        .bind(&all_types)
                        .bind(&all_serialized)
                        .bind(Self::current_context())
                        .fetch_all(op.as_executor())
                        .await
                )?;
