...
/// The generated list queries must produce query *plans* that can use an
/// index: with seq scans disabled, a sargable predicate shows up as an
/// `Index Cond`, while the legacy `COALESCE(col = $1, $1 IS NULL)` catch-all
/// can only ever be a `Filter` on top of a full (index or seq) scan.
#[tokio::test]
async fn specialized_queries_plan_with_index_cond() -> anyhow::Result<()> {
    let pool = helpers::init_pool().await?;
    let transfers = Transfers::new(pool.clone());

    let account_ids: Vec<uuid::Uuid> = (0..5).map(|_| uuid::Uuid::from(AccountId::new())).collect();
    let specs: Vec<_> = (0..200)
        .map(|i| (account_ids[i % account_ids.len()], "plan_test", None, None))
        .collect();
    ...
    assert!(
        ...
        "specialized page-1 query must use an index condition, got plan:\n{plan}"
    );
    ...
    assert!(
        plan.contains("Index Cond"),
        "specialized cursor query must use an index condition, got plan:\n{plan}"
    );

    // The legacy catch-all (kept as fallback for filter combinations beyond
