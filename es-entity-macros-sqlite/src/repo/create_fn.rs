...
impl ToTokens for CreateFn<'_> {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        ...
        let query = format!(
            "INSERT INTO {} ({}, created_at) VALUES ({}, COALESCE(?{}, datetime('now')))",
            table_name,
            column_names.join(", "),
            placeholders,
            column_names.len() + 1,
        ...
        let expected = quote! {
                ...
                OP: es_entity::AtomicOperation
            {
                let __result: Result<Entity, EntityCreateError> = async {
                    ...
                    sqlx::query("INSERT INTO entities (id, created_at) VALUES (?1, COALESCE(?2, datetime('now')))")
                        .bind(id)
                        .bind(op.maybe_now())
                    .execute(op.as_executor())
                    .await
        ...
        let expected = quote! {
                ...
                OP: es_entity::AtomicOperation
            {
                let __result: Result<Entity, EntityCreateError> = async {
                    ...
                    sqlx::query("INSERT INTO entities (id, name, created_at) VALUES (?1, ?2, COALESCE(?3, datetime('now')))")
                        .bind(id)
                        .bind(name)
                        .bind(op.maybe_now())
                    .execute(op.as_executor())
