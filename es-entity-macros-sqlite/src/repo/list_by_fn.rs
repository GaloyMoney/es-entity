...
impl CursorStruct<'_> {
    ...
    pub fn condition(&self, offset: u32, ascending: bool) -> String {
        ...
        if self.column.is_id() {
            ...
            format!(
                "({0} IS ${column_offset}) AND COALESCE(id {comp} ${id_offset}, true) OR COALESCE({0} {comp} ${column_offset}, {0} IS NOT NULL)",
                self.column.name(),
            )
        } else {
            format!(
                ...
                let __result: Result<es_entity::PaginatedQueryRet<Entity, cursor_mod::EntitiesByValueCursor>, EntityQueryError> = async {
                    ...
                    let (entities, has_next_page) = match direction {
                        es_entity::ListDirection::Ascending => {
                            es_entity::es_query!(
                                entity = Entity,
                                "SELECT value, id FROM entities WHERE ((value IS $3) AND COALESCE(id > $2, true) OR COALESCE(value > $3, value IS NOT NULL)) ORDER BY value ASC NULLS FIRST, id ASC LIMIT $1",
                                (first + 1) as i64,
                                id as Option<EntityId>,
                        ...
                        es_entity::ListDirection::Descending => {
                            es_entity::es_query!(
                                entity = Entity,
                                "SELECT value, id FROM entities WHERE ((value IS $3) AND COALESCE(id < $2, true) OR COALESCE(value < $3, value IS NOT NULL)) ORDER BY value DESC NULLS LAST, id DESC LIMIT $1",
                                (first + 1) as i64,
                                id as Option<EntityId>,
                                value as Option<rust_decimal::Decimal>,
                            )
