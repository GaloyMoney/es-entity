{
  ...
  outputs = {
    ...
    (system: let
      ...
      # same script runs locally (`nix run .#fuzz`) and in the Concourse `fuzz`
      # job. The targets are pure — no Postgres needed. FUZZ_SECONDS etc. are
      # passed through the environment; see ci/fuzz.sh.
